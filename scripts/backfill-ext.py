...
WRANGLER_CWD: str = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'backend', 'web')
...
def run_query(sql: str) -> list[dict] | None:
    ...
    if result.returncode != 0:
        log.error('  DB error: %s', result.stderr[:300])
        return None
    try:
...
def run_update(sql: str) -> bool:
    ...
    if not ok:
        log.error('  UPDATE fail: %s', result.stderr[:300])
    return ok

def ollama_generate(prompt: str, timeout: int = 300) -> str | None:
...
def split_turns(content: str) -> list[str]:
...
def summarize_chunk(text: str, ci: int, total: int, retries: int = 2) -> tuple[dict | None, str]:
    ...
    for attempt in range(retries + 1):
        ...
        if attempt < retries:
            log.warning('%s: RETRY in 5s...', label)
            time.sleep(5)
        else:
...
def generate_final(chunk_results: list[tuple[dict, str]]) -> dict | None:
...
def generate_checkpoint(fm: dict) -> str:
...
def process_note(note_id: int, title: str, content: str) -> bool:
    ...
    if len(turns) < 2:
        log.warning('  Skip: too few turns')
        return False

    chunks = chunk_turns(turns)
    log.info('  Turns: %d, Chunks: %d', len(turns), len(chunks))

    chunk_results = []
    for i, ch in enumerate(chunks):
        log.info('  Chunk %d/%d (%s chars)...', i + 1, len(chunks), f'{len(ch["text"]):,}')
    ...
    if not fm:
        log.error('  Final summary FAIL')
        return False
    log.info('  Final summary OK')

    log.info('  Checkpoint...')
    cp = generate_checkpoint(fm)
    log.info('  Checkpoint %s', 'OK' if cp else 'EMPTY')
    ...
    log.info('  DONE: %s | %s', fm.get('project', ''), fm.get('summary', '')[:60])
    return True
...
def main() -> None:
    ...
    """
    log.info('=' * 60)
    log.info('  ext_sessions backfill (강화 프롬프트)')
    log.info('=' * 60)
    ...
    log.info('Already in ext_sessions: %d', len(done_ids))
    ...
    log.info('Total notes: %d, Done: %d, Todo: %d', len(rows), len(done_ids), len(todo))
    ...
    for i, row in enumerate(todo):
        log.info('\n[%d/%d] id=%d %s chars - %s', i + 1, len(todo), row['id'], f'{row["size"]:,}', row['title'][:50])
    ...
    log.info('\n' + '=' * 60)
    log.info('  Done: %d ok / %d fail / %d total', ok, fail, len(todo))
    log.info('=' * 60)
